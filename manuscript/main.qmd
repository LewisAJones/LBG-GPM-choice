...
Subsequent data manipulation and visualisation was carried out using the Tidyverse suite of R packages [@wickham2019]. The raw number of genera was counted per stage per palaeolatitudinal bin for each Global Plate Model. We also estimated the latitudinal biodiversity gradient using coverage-based interpolation and extrapolation of Hill numbers, in the R package iNEXT [@hsieh2016]. The rarefaction portion of this approach is equivalent to shareholder quorum subsampling [@alroy2010; @close2018]. Extrapolated values with an estimated sample size above twice that observed were discarded [@hsieh2016].

- Will and Lucas to add visualisation and curve comparison
